...
class HomePresenterImpl(var homeView: HomeView?) : HomePresenter, ResponseHandler<List<HomeItemBean>> {
    override fun loadDatas() {
        Log.d("TAG", "loadDatas====")
        HomeRequest(HomePresenter.TYPE_INIT_OR_REFRESH, 0, this).exute()
    }

    override fun loadMore(offset: Int) {
        HomeRequest(HomePresenter.TYPE_LOAD_MORE, offset, this).exute()
    }
    ...
    override fun onSucess(type: Int, response: List<HomeItemBean>) {
        when (type) {
            //初始化或者刷新数据
            HomePresenter.TYPE_INIT_OR_REFRESH -> homeView?.loadSucess(response)
            //加载更多
            HomePresenter.TYPE_LOAD_MORE -> homeView?.loadMore(response)
        }
    }
}